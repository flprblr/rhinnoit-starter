...
<template>
    ...
    <DropdownMenuGroup>
        <DropdownMenuItem :as-child="true" v-if="$can('users.index')">
            <Link class="block w-full" :href="maintainers.users.index().url" prefetch as="button">
                <UserRound class="mr-2 h-4 w-4" />
                Users
            </Link>
        </DropdownMenuItem>
        <DropdownMenuItem :as-child="true" v-if="$can('roles.index')">
            <Link class="block w-full" :href="maintainers.roles.index().url" prefetch as="button">
                <UserRoundCog class="mr-2 h-4 w-4" />
                Roles
            </Link>
        </DropdownMenuItem>
        <DropdownMenuItem :as-child="true" v-if="$can('permissions.index')">
            <Link class="block w-full" :href="maintainers.permissions.index().url" prefetch as="button">
